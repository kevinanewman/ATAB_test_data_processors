function create_legend_page(engine, data_types, image_path, out_file, include_x, title_append, show_figs )
%creates a .pdf page within the current directory
%if there is a legend_image.*** image file, it will be inserted

sugg_citation = engine.citation;
...
t.String = sprintf('%s %s - %s Data Plots', engine.name,engine.fuel, title_append);
...
%% WOT citation
if ~isempty( engine.source_citation )
	t = annotation('textbox');
	t.String = sprintf('Published maximum torque data from:\n%s',engine.source_citation);
