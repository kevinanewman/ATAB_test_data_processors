function [filepath, filename] = datagrabber(path, varargin)
...
%	Parameter Options:
%
%		Select			Select column headings to import, its value must be a column
%						cell array of strings. Wildcard * characters are supported.
%
%		Exclude			Select column headings to exclude from importation, its value
%						must be a column cell array of strings. Wildcard * characters
%						are supported.
%
%		Rename			Select column headings to rename during importation, its value
...
%
%       HeaderLine		Line number of column headings to use for extraction.
%
%		DataLine		Line number where data for extraction begins.
%
%		EndLine			Line number where data extraction ends.
%
%       UnitLine        Line number where the unit strings are.
...
%                       structure (example: structname.unit.varname =
%                       'unitstring')
%
%		Delimiter		Delimiter character that will be used to separate entries.
%
%		Sheet			Specify sheet within Excel workbook. It can be a numeric
%						specifier or a string corresponding to the name of the
%						requested worksheet.
%
%		Structure		Imports the selected data into fields of a Matlab structure
%						in the base workspace.
%
%       Quiet           Run without printing status information to the
%                       console.
%
%


% ex: datagrabber('P310798_Malibu_01-20-13_HWFE_Test_4.txt',{'TimeStamp','time';'VSPD_MPH','vspeed'})
...
unit_line       = parse_varargs(varargin,'unitline',[]);
...
%    evalin('caller',['clear ' structure]);
    evalin('caller',[structure '= struct();']);
    structure(end+1) = '.';
end
