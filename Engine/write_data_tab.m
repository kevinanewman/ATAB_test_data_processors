function write_data_tab( xls, sheet, data, engine, select)
...
% Title Block
xls.write({[engine.name, ' Engine ', engine.fuel, ' - ', select, ' Data']},'A1')
xls.format(xlsrange( 'A', 1, num_cols-1), 'MergeCells',true,'FontSize',16,'FontBold',true);

% Fuel Block
xls.write({engine.fuel_description},'A2')
xls.format(xlsrange( 'A', 2, num_cols-1), 'MergeCells',true);

% Citation Block
xls.write({engine.citation},'A3')
