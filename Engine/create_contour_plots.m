...
for var_idx = 1:width(indata)
	...
	if contains(zvar,'bsfc_gpkWhr')
		surfZ = surfZ ./ ( xmesh .* convert.rpm2radps .* ymesh ./1000 ) * 3600;
		surfZ(ymesh <= 0 ) = 1.5*max(zdata);
		surfZ = min(surfZ, 1.5*max(zdata));
	elseif contains(zvar,'bte_pct')
		surfZ =  100 * ( xmesh .* convert.rpm2radps .* ymesh ./ 1000 ) ./ ( surfZ .* e.fuel_heating_val_MJpkg );
		surfZ(ymesh <= 0 ) = 0;
		surfZ = max(surfZ, 0);
	end
...
% 	saveas( [out_file{var_idx},'.fig'] );
	...
	%% Save EMF
	print_emf([out_file{var_idx},'.emf'],[6,4.5]);
