function engine_data_packet_builder( format_file , show_figs)
...
if ~ismember( 'Engine Spec', format_sheets)
	error('Invalid Template File - Missing "Engine Spec" Tab');
...
fprintf('Reading Engine Specifications\n');
waitbar(0, waitbar_hand, 'Parsing Template - Engine Spec');
engine = read_engine_spec(format_xls);

%% Set Output Folder & Clean it out
out_name = [engine.name,' Engine ', engine.fuel];
...
[inj_cal, inj_cal_pdf, inj_cal_emf] = compute_injector_cal(  merged_data , engine, show_figs, out_fldr, waitbar_hand);
...
if ~isempty( inj_cal) 	
	...
	merged_data  = add_injector_flow( merged_data, inj_cal, engine );
	...
	data_format(inj_idx).calibration_uncertainty = sqrt( data_format(meter_idx).calibration_uncertainty.^2 + ( inj_cal.fit_uncertainty ).^2 );
...
if ~isempty( max_files)	
	...
	raw_data = add_injector_flow( raw_data, inj_cal, engine );
	raw_data = add_continuous_calcs( raw_data, engine);

	[raw_data, data] = process_max_torque( raw_data, data_format, data_files(max_files).file, engine, show_figs );
...
if ~isempty( min_files )
	...
	[raw_data, data] = process_min_torque( raw_data, data_format, data_files(min_files).file, engine, show_figs );
...
if ~isempty(hl_files)
	...
	if isnan(engine.max_fuel_press ) && istablevar( merged_data, 'fuel_rail_press')
		engine.max_fuel_press = max( merged_data.fuel_rail_press) * 1.15 ;
	end
	...
	for d = hl_files
		...
		data = process_high_load(raw_data, data_format, data_files(d).file, data_files(d).data_type, engine, show_figs ) ;
...
merged_data = add_base_calcs(merged_data, data_format, engine);
waitbar(0.3, waitbar_hand);
...
plot_bound.min_speed_rpm = 0.95 * min( min(merged_data.speed_rpm), engine.idle_speed_rpm);
plot_bound.max_speed_rpm = 1.05* max( max( merged_data.speed_rpm), engine.max_power_speed_rpm);
plot_bound.max_torque_Nm = 1.05 * max( max(merged_data.torque_Nm), engine.max_torque_Nm);
...
for l = 1:length(template_labels)
	...
	if any(sel_tests)
		...
		write_data_tab( out_xls, 'Steady State', sel_data, engine, out_file_label);
	end
	...
	if any(sel_tests)
		...
		sel_data = sort_data_rows( select_data(sel_tests,:) );
		sel_data = remove_blank_columns( sel_data );
		write_data_tab( out_xls, 'Steady State with Transmission', sel_data, engine, out_file_label);
	end
	...
	if any(sel_tests)
		...
		sel_data = sort_data_rows(select_data(sel_tests,:));
		sel_data = remove_blank_columns( sel_data );
		write_data_tab( out_xls, 'High Load Initial', sel_data, engine, out_file_label);
	end
	...
	if any(sel_tests)
		...
		sel_data = sort_data_rows(select_data(sel_tests,:));
		sel_data = remove_blank_columns( sel_data );
		write_data_tab( out_xls, 'High Load Final', sel_data, engine, out_file_label);
	end
	...
	if any(sel_tests)
		...
		write_data_tab( out_xls, 'Max Torque Sweep', sel_data, engine, out_file_label);
	end
	...
	if any(sel_tests) && strcmpi( template_label, 'FULL' ) 
		...
		sel_data = sort_data_rows(select_data(sel_tests,:));
		sel_data = remove_blank_columns( sel_data );
		write_data_tab( out_xls, 'Max Torque Sweep - Raw', sel_data, engine, out_file_label);
	end
	...
	if any(sel_tests)
		...
		sel_vars = ismember(select_data.Properties.VariableNames, {'speed_rpm','torque_Nm','bmep_bar','meter_fuel_flow_gps','injector_fuel_flow_gps'} );
		sel_data = sort_data_rows(select_data(sel_tests,sel_vars));
		write_data_tab( out_xls, 'Min Torque Sweep', sel_data, engine, out_file_label);
	end
	...
	if any(sel_tests) && strcmpi( template_label, 'FULL' ) 
		...
		sel_data = sort_data_rows(select_data(sel_tests,:));
		sel_data = remove_blank_columns( sel_data );
		write_data_tab( out_xls, 'Min Torque Sweep - Raw', sel_data, engine, out_file_label);
	end
    ...
    write_param_tab( out_xls, 'Test Parameter List', select_data, engine, out_file_label)
...
for c = 1:length(contour_sets)
    ...
    % Make contour plots
	out_pdf = create_contour_plots(merged_data.speed_rpm(sel_points), merged_data.torque_Nm(sel_points), merged_data.data_type(sel_points), plot_data(sel_points,:), plot_bound, cont_fldr, engine, max_torque_data, min_torque_data, contour_set.label, show_figs);
...
%% Make plots of test data points & test number & mode number
	...
	ax = plot_data_points(merged_data.data_type(sel_pts),  merged_data.speed_rpm(sel_pts), merged_data.torque_Nm(sel_pts), engine, plot_bound, show_figs  );
	...
	ax = plot_data_points(merged_data.data_type,  merged_data.speed_rpm, merged_data.torque_Nm, engine, plot_bound, show_figs  );
...
for l = 1:length(out_file_labels)
	...
	% Create Legend
	create_legend_page(engine, legend_data_types,  format_path ,legend_pdf,  true, out_file_label, show_figs );	
...
function e = read_engine_spec(format_xls)


format_xls.change_sheet('Engine Spec');
...
parse_format = cell2struct({  
	...
	'Displacement (L)'				'displacement_L'			'numeric'		true
	'Engine Model / Family'			'model'						'text'			true
	'Number of Cylinders'			'num_cylinders'				'numeric'		true
	'Fuel Name'						'fuel'						'text'			false
	'Fuel Description'				'fuel_description'			'text'			false
	'Fuel Heating Value (MJ/kg)'	'fuel_heating_val_MJpkg'	'numeric'		false
	'Fuel Carbon Fraction'			'fuel_carbon_mass_norm'		'numeric'		false
	'Idle Speed (RPM)'				'idle_speed_rpm'			'numeric'		false
	'Max Power (kW)'				'max_power_kW'				'numeric'		false
	'Max Power Speed (RPM)'			'max_power_speed_rpm'		'numeric'		false
	'Max Torque (Nm)'				'max_torque_Nm'				'numeric'		false
	'Max Torque Speed (RPM)'		'max_torque_speed_rpm'		'numeric'		false
	'Max Fuel Rail Pressure',		'max_fuel_press',			'numeric',		false
	'Source Data Citation'			'source_citation'			'text'			false
	'Intake Valve Open (deg)'		'intake_valve_open_deg'		'numeric'		false
	'Intake Valve Close (deg)'		'intake_valve_closed_deg'	'numeric'		false
	'Exhaust Valve Open (deg)'		'exhaust_valve_open_deg'	'numeric'		false
	'Exhaust Valve Close (deg)'		'exhaust_valve_closed_deg'	'numeric'		false
	'Bore Diameter (mm)'			'bore_diameter_mm'			'numeric'		false
	'Stroke Length (mm)'			'stroke_length_mm'			'numeric'		false
	'Crank Offset (mm)'				'crank_offset_mm'			'numeric'		false
	'Connecting Rod Length (mm)'	'connecting_rod_length_mm'	'numeric'		false
	'Clearance Volume (cc)'			'clearance_volume_cc'		'numeric'		false
	'Message'						'message'					'text'			false }, ...
	{'header',						'out_var',				'type',		'required'}, 2);

e = template_parser(parse_format, read_txt, 'Engine Spec');



%% map input table data to engine struct
