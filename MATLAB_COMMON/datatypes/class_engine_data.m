classdef class_engine_data
    %class_engine_data
    %   Holds engine data as specified by GUNC
    ...
    properties
        ...
        fuel = class_fuel_data				% engine fuel data 
		fuel_meter = class_fuel_data		% engine fuel data
		fuel_injector = class_fuel_data     % engine fuel data
    ...
    methods
        ...
        function obj = set.speed_rpm( obj, val )
            obj.speed_radps = convert.rpm2radps * val ;
        end
        
        function val = get.speed_rpm( obj )
            val = convert.radps2rpm * obj.speed_radps;
        end
        
        function val = get.accel_radps2( obj )
            val = derivative(obj.speed_radps, 1)./derivative(obj.time, 1);
        end
                        
        function val = get.gross_power_hp( obj )
            val = convert.kW2hp * obj.gross_power_kW;
        end
                
        function val = get.crankshaft_power_hp( obj )
            val = convert.kW2hp * obj.crankshaft_power_kW;
        end
        
        function obj = set.gross_torque_ftlbs( obj, val )
            obj.gross_torque_Nm = convert.ftlbs2Nm * val;
        end
        
        function val = get.gross_torque_ftlbs( obj )
            val = convert.Nm2ftlbs * obj.gross_torque_Nm;
        end
        
        function obj = set.crankshaft_torque_ftlbs( obj, val )
            obj.crankshaft_torque_Nm = convert.ftlbs2Nm * val;
        end
        
        function val = get.crankshaft_torque_ftlbs( obj )
            val = convert.Nm2ftlbs * obj.crankshaft_torque_Nm;
        end
        ...
        function val = get.coolant_temp_degF( obj )
            val = convert.degC2degF( obj.coolant_temp_degC );
        end
        
        function obj = set.coolant_temp_degF( obj, val )
            obj.coolant_temp_degC = convert.degF2degC(val);
        end
